...
use zellij_utils::cli::{Sessions, SessionCommand};
use zellij_utils::input::actions::Action;
use zellij_utils::input::config::{ConfigError, Config};
...
use russh::{CryptoVec, ChannelId, Sig};
use crate::zellij_session::{ActiveSession, get_active_session, SessionNameMatch, list_sessions, get_sessions_sorted_by_mtime, match_session_name};
use tokio::sync::mpsc::*;
...
impl Session {
    ...
    pub async fn run(&mut self) {
        loop {
            if let Some(event) = self.rx.recv().await {
                self.handle_handler_event(event, self.zellij_cli_args.clone()).await
            }
        }
    }
    ...
    pub async fn recv(&mut self) {
        ...
        let channel_id = self.channel_id.clone().unwrap().0;
        loop {
        ...
        match event {
            ...
            HandlerEvent::ShellRequest(channel_id) => {
                ...
                std::thread::spawn( move || {
                    Self::start_zellij_client(args, result, sender, server_receiver, server_signal_receiver, ServerHandle(handle), channel_id.0, win_size);
                });
            ...
            HandlerEvent::WindowChangeRequest(_, winsize ) => {
    ...
    fn start_zellij_client(args: CliArgs, pty: OpenptyResult, sender: UnboundedSender<(Option<String>, Option<()>)>, server_receiver: crossbeam_channel::Receiver<Vec<u8>>,
        server_signal_receiver: crossbeam_channel::Receiver<Sig>, handle: ServerHandle, channel_id: ChannelId, win_size: Winsize
    ) {
        use zellij_utils::cli::Command; use zellij_utils::data::ConnectToSession;
        ...
        let zellij_cli_args = zellij_utils::cli::CliArgs {
            ...
            command: Some(Command::Sessions(Sessions::Attach { session_name: args.session.clone(), create: false, force_run_commands: false, index: None, options: None})),
            debug: args.debug,
        };
        ...
        let os_input = get_server_input(handle, channel_id, win_size, sender.clone(), server_receiver, server_signal_receiver);
        //let handle = self.handle.clone().unwrap();
        ...
        //loop {
            ...
            let mut opts = zellij_cli_args.clone();
            ...
            })) = opts.command.clone()
            {
                ...
                if let Some(session_name) = opts.session.clone() {
                   ...
                   // let session_name = "fff".to_string();
                   //start_client_ssh(
                   //     Box::new(os_input),
                   //     opts,
                   //     config,
...
fn get_server_input(handle: ServerHandle, channel_id: ChannelId, win_size: Winsize, sender: UnboundedSender<(Option<String>, Option<()>)>, 
    server_receiver: crossbeam_channel::Receiver<Vec<u8>>, 
    server_signal_receiver: crossbeam_channel::Receiver<Sig>) -> SshInputOutput {
